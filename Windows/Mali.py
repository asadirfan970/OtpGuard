...
def main():
    ...
    try:
        ...
        # Search for the specific country - handle both +223 and 223 formats
        # Remove + from country_code if present to match MICO app format
        clean_code = country_code.replace("+", "") if country_code.startswith("+") else country_code
        country_search_text = f"{country_name}  ({clean_code})"
        ...
        for i in range(30):  # Increased scroll attempts
            ...
            if potential.exists:
                ...
                d(scrollable=True).scroll(steps=50)  # Increased scroll steps like test.py
                time.sleep(1)
